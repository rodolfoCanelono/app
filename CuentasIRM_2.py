...
# =========================================================
st.title("📊 Sistema de Inteligencia de Gastos - Rodolfo Canelón")
st.markdown("---")

# Definición de las 4 pestañas solicitadas
tab1, tab2, tab3, tab4 = st.tabs(["📝 Registro", "📈 Dashboard", "⚖️ Cuadres - Aportes", "🔮 Análisis y Pronóstico"])
...
with tab2:
    if not df.empty:
        st.subheader("🔍 Filtros y Resumen de Selección")
        ...
        with g2:
            ...
            fig2 = px.pie(df_r, values='monto', names='responsable', hole=0.4, title="Distribución por Responsable")
...
# --- PESTAÑA 3: CUADRES - APORTES (NUEVA PESTAÑA SOLICITADA) ---
with tab3:
    if not df.empty:
        st.subheader("⚖️ Inteligencia de Aportes y Cuadres")
        df_temp = df.copy()
        df_temp['mes_año'] = df_temp['fecha'].dt.strftime('%Y-%m')

        # 1. Cuadre Mensual por Responsable
        st.write("### 📑 Cuadre Mensual de Gastos")
        ...
        # 2. Distribución % Histórica de Responsables
        col_c1, col_c2 = st.columns(2)
        with col_c1:
            st.write("### 💰 Participación en el Aporte")
            total_hist = df_temp.groupby('responsable')['monto'].sum().reset_index()
            suma_total = total_hist['monto'].sum()
            total_hist['% Participación'] = (total_hist['monto'] / suma_total) * 100
            
            fig_pie_cuadre = px.pie(total_hist, values='monto', names='responsable', hole=0.5)
        ...
        st.info("Registre datos para visualizar el cuadre de aportes.")

# --- PESTAÑA 4: ANÁLISIS Y PRONÓSTICO ---
with tab4:
    if not df.empty:
        st.subheader("🔮 Evolución Temporal y Proyecciones")
        df_temp = df.copy().sort_values('fecha')
        df_temp['mes_año'] = df_temp['fecha'].dt.strftime('%Y-%m')

        # Pronóstico basado en promedios mensuales
        totales_mes_p = df_temp.groupby('mes_año')['monto'].sum().reset_index()
        promedio_p = totales_mes_p['monto'].mean()
        
        st.info(f"El gasto promedio mensual real es de: **${promedio_p:,.0f}**")
        ...
        fig_p_bar = px.bar(df_final_p, x='mes_año', y='monto', color='Tipo', text_auto='.2s', 
                           title="Historial de Flujo vs Pronóstico a 3 Meses",
...
st.sidebar.write(f"Responsables: {len(LISTA_RESPONSABLES)}")
st.sidebar.write(f"Conceptos: {len(LISTA_CONCEPTOS)}")
