...
# --- 3. FUNCIONES DE CARGA DINÁMICA (DB) ---

def cargar_lista_db(tabla, columna, respaldo):
    """Carga listas dinámicas desde Supabase (Responsables o Conceptos)"""
    try:
        ...
        st.sidebar.warning(f"Error cargando {tabla}: {e}")
        return respaldo
...
def cargar_datos_db():
    ...
    try:
        ...
        if not df_raw.empty:
            # Tipado numérico estricto para evitar el error del 33% en gráficas
...
# Ahora cargamos Responsables y Conceptos directamente de la DB
...
with tab1:
    ...
    with st.form("form_registro", clear_on_submit=True):
        ...
        with col_reg2:
            ...
            responsable_in = st.selectbox("Responsable", LISTA_RESPONSABLES)
        
        if st.form_submit_button("Guardar Gasto"):
            if guardar_gasto_db(fecha_in, concepto_in, monto_in, responsable_in, forma_pago_in):
                st.success(f"✅ Gasto de {responsable_in} guardado.")
                st.rerun()
...
with tab2:
    if not df.empty:
        ...
        # Resumen y División de Gastos Actualizado
        ...
        # Gráficas de Torta con Suma Real
        st.markdown("---")
        g1, g2 = st.columns(2)
        with g1:
            df_c_pie = df_f.groupby('concepto')['monto'].sum().reset_index()
            fig1 = px.pie(df_c_pie, values='monto', names='concepto', hole=0.4, title="Por Concepto")
            fig1.update_traces(textinfo='percent+value', texttemplate='%{percent}<br>$%{value:,.0f}')
            st.plotly_chart(fig1, use_container_width=True)
        with g2:
            df_r_pie = df_f.groupby('responsable')['monto'].sum().reset_index()
            fig2 = px.pie(df_r_pie, values='monto', names='responsable', hole=0.4, title="Por Responsable")
...
with tab3:
    if not df.empty:
        ...
        st.info(f"El gasto promedio mensual real es de: **${promedio_mensual_real:,.0f}**")
        ...
        fig_pron = px.bar(df_final, x='mes_año', y='monto', color='Tipo', text_auto='.2s',
                         title="Historial vs Proyección de Gasto",
