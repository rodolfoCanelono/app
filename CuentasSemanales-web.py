...
with tab1:
    ...
    with st.form("form_reg", clear_on_submit=True):
        ...
        with c1:
            ...
            mon_in = st.number_input("Monto", min_value=2000, step=1000, format="%d")
...
with tab3:
    if not df.empty:
        ...
        # 2. Distribución % Histórica
        st.subheader("💰 Participación Total en el Gasto")
        total_hist = df_temp.groupby('responsable')['monto'].sum().reset_index()
        
        col_pie1, col_pie2 = st.columns(2)
        with col_pie1:
            fig_pie_h = px.pie(total_hist, values='monto', names='responsable', hole=0.5, title="Capital Aportado")
